...
// ターゲット環境でSSE,AVX,AVX2が搭載されていない場合はこれらの命令をsoftware
// emulationにより実行する。 software
// emulationなので多少遅いが、SSE2,SSE4.1,SSE4.2,AVX,AVX2,AVX-512の使えない環境でそれに合わせたコードを書く労力が省ける。
...
// PEXT命令を使わず、この下にあるsoftware
// emulationによるPEXT実装を用いたほうがまだマシらしい。(どうなってんの…)
...
#define PEXT64(a, b)                                       \
  (u64(PEXT32((a) >> 32, (b) >> 32) << (u32)POPCNT32(b)) | \
   u64(PEXT32(u32(a), u32(b))))
#endif

#else

// for non-BMI2 : software
// emulationによるpext実装(やや遅い。とりあえず動くというだけ。)
// ただし64-bitでもまとめて処理できる点や、magic
// bitboardのような巨大テーブルを用いない点において優れている(かも)
inline uint64_t pext(uint64_t val, uint64_t mask) {
  uint64_t res = 0;
  for (uint64_t bb = 1; mask; bb += bb) {
    if ((int64_t)val & (int64_t)mask & -(int64_t)mask) res |= bb;
...
inline int32_t POPCNT64(uint64_t a) {
  a = (a & UINT64_C(0x5555555555555555)) +
      (a >> 1 & UINT64_C(0x5555555555555555));
  a = (a & UINT64_C(0x3333333333333333)) +
      (a >> 2 & UINT64_C(0x3333333333333333));
  a = (a & UINT64_C(0x0f0f0f0f0f0f0f0f)) +
      (a >> 4 & UINT64_C(0x0f0f0f0f0f0f0f0f));
  a = (a & UINT64_C(0x00ff00ff00ff00ff)) +
      (a >> 8 & UINT64_C(0x00ff00ff00ff00ff));
  a = (a & UINT64_C(0x0000ffff0000ffff)) +
...
FORCE_INLINE int LSB64(uint64_t v) {
  ASSERT_LV3(v != 0);
  return uint32_t(v) ? LSB32(uint32_t(v)) : 32 + LSB32(uint32_t(v >> 32));
}
...
FORCE_INLINE int MSB64(uint64_t v) {
  ASSERT_LV3(v != 0);
  return uint32_t(v >> 32) ? 32 + MSB32(uint32_t(v >> 32)) : MSB32(uint32_t(v));
}
#endif
...
FORCE_INLINE int LSB32(const u32 v) {
  ASSERT_LV3(v != 0);
  return __builtin_ctzll(v);
}
FORCE_INLINE int LSB64(const u64 v) {
  ASSERT_LV3(v != 0);
  return __builtin_ctzll(v);
}
FORCE_INLINE int MSB32(const u32 v) {
  ASSERT_LV3(v != 0);
  return 63 ^ __builtin_clzll(v);
}
FORCE_INLINE int MSB64(const u64 v) {
  ASSERT_LV3(v != 0);
  return 63 ^ __builtin_clzll(v);
}
...
FORCE_INLINE int pop_lsb(u64& b) {
  int index = LSB64(b);
  b = BLSR(b);
  return index;
}
