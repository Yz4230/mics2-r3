...
template <MOVE_GEN_TYPE GenType, Piece Pt, Color Us, bool All>
...
struct GeneratePieceMoves<GenType, PAWN, Us, All> {
  FORCE_INLINE ExtMove *operator()(const Position &pos, ExtMove *mlist,
                                   const Bitboard &target) {
    // 盤上の自駒の歩に対して
...
template <MOVE_GEN_TYPE GenType, Color Us, bool All>
struct GeneratePieceMoves<GenType, GPM_BR, Us, All> {
  FORCE_INLINE ExtMove *operator()(const Position &pos, ExtMove *mlist,
                                   const Bitboard &target) {
    // 角と飛に対して(馬と龍は除く)
...
template <MOVE_GEN_TYPE GenType, Color Us, bool All>
struct GeneratePieceMoves<GenType, GPM_GHDK, Us, All> {
  FORCE_INLINE ExtMove *operator()(const Position &pos, ExtMove *mlist,
                                   const Bitboard &target) {
    // 金相当の駒・馬・龍・玉に対して
...
template <MOVE_GEN_TYPE GenType, Color Us, bool All>
struct GeneratePieceMoves<GenType, GPM_GHD, Us, All> {
  FORCE_INLINE ExtMove *operator()(const Position &pos, ExtMove *mlist,
                                   const Bitboard &target) {
    // 金相当の駒・馬・龍に対して
...
struct GenerateDropMoves {
  ExtMove *operator()(const Position &pos, ExtMove *mlist,
    ...
    if (hand_exists(hand, PAWN)) {
      ...
      // 歩の打てる場所
      Bitboard target2 = rank1_n_bb(~Us, RANK_4) & target;

      // 歩が二歩のために打てない筋を消していく(Aperyの手法)
      Bitboard pawnBB = pos.pieces(Us, PAWN);
      Square pawnSq;
      foreachBB(pawnBB, pawnSq,
    ...
    if (hand_except_pawn_exists(hand)) {
      ...
      int num = 0;
      if (hand_exists(hand, SILVER))
        drops[num++] = make_move_drop(SILVER, SQ_ZERO) + OurDropPt(Us, SILVER);
      if (hand_exists(hand, GOLD))
        drops[num++] = make_move_drop(GOLD, SQ_ZERO) + OurDropPt(Us, GOLD);
      if (hand_exists(hand, BISHOP))
        drops[num++] = make_move_drop(BISHOP, SQ_ZERO) + OurDropPt(Us, BISHOP);
      if (hand_exists(hand, ROOK))
        drops[num++] = make_move_drop(ROOK, SQ_ZERO) + OurDropPt(Us, ROOK);
...
ExtMove *generate_evasions(const Position &pos, ExtMove *mlist) {
  ...
  while (bb) {
    Square to = bb.pop();
    mlist++->move = make_move(ksq, to) + OurPt(Us, KING);
  }
  ...
  // target2 == 移動による指し手は1)+2) = 王手している駒と王との間の升 +
  // 王手している駒　の升
  ...
  mlist =
      GeneratePieceMoves<NON_EVASIONS, PAWN, Us, All>()(pos, mlist, target2);
  mlist =
      GeneratePieceMoves<NON_EVASIONS, SILVER, Us, All>()(pos, mlist, target2);
  mlist =
...
ExtMove *generate_general(const Position &pos, ExtMove *mlist,
  ...
  static_assert(GenType != EVASIONS_ALL && GenType != NON_EVASIONS_ALL &&
                    GenType != RECAPTURES_ALL,
                "*_ALL is not allowed.");
  ...
  // 各駒による移動の指し手の生成
  // 歩の指し手は歩のBitboardをbit
  // shiftすることで移動先が一発で求まるので特別扱い
  mlist = GeneratePieceMoves<GenType, PAWN, Us, All>()(pos, mlist, targetPawn);

  // 銀は成れるなら成らない手の他に成る手も生成する駒。これらによる移動の指し手
  ...
  if (GenType == NON_CAPTURES || GenType == NON_CAPTURES_PRO_MINUS ||
      GenType == NON_EVASIONS)
...
ExtMove *make_move_target_pro(Square from, const Bitboard &target,
                              ExtMove *mlist) {
  auto bb = target;
  while (bb) {
    auto to = bb.pop();
...
template <Color Us, bool All>
ExtMove *make_move_check(const Position &pos, Piece pc, Square from, Square ksq,
                         const Bitboard &target, ExtMove *mlist) {
...
ExtMove *generate_checks(const Position &pos, ExtMove *mlist) {
  ...
  // 集合としては1),2) <-->
  // 3)は被覆している可能性があるのでこれを除外できるような指し手生成をしなくてはならない。
  // これを綺麗に実装するのは結構難しい。
  ...
  while (src) {
    ...
    mlist = make_move_target_general<Us, All>()(pos, pos.piece_on(from), from,
                                                target & ~pin_line, mlist);

    if (x & from)
      // 直接王手にもなるので↑で生成した~line_bb以外の升への指し手を生成。
      mlist = make_move_check<Us, All>(pos, pos.piece_on(from), from, themKing,
                                       target & pin_line, mlist);
  }
  ...
  if (hand_exists(h, PAWN))
    mlist = GenerateCheckDropMoves<Us, PAWN>()(
        pos, pos.check_squares(PAWN) & empties, mlist);
  if (hand_exists(h, SILVER))
    mlist = GenerateCheckDropMoves<Us, SILVER>()(
        pos, pos.check_squares(SILVER) & empties, mlist);
  if (hand_exists(h, GOLD))
    mlist = GenerateCheckDropMoves<Us, GOLD>()(
        pos, pos.check_squares(GOLD) & empties, mlist);
  if (hand_exists(h, BISHOP))
    mlist = GenerateCheckDropMoves<Us, BISHOP>()(
        pos, pos.check_squares(BISHOP) & empties, mlist);
  if (hand_exists(h, ROOK))
    mlist = GenerateCheckDropMoves<Us, ROOK>()(
        pos, pos.check_squares(ROOK) & empties, mlist);
...
ExtMove *generateMoves(const Position &pos, ExtMove *mlist, Square sq = SQ_NB) {
  return pos.side_to_move() == BLACK
             ? generate_general<GenType, BLACK, All>(pos, mlist, sq)
             : generate_general<GenType, WHITE, All>(pos, mlist, sq);
}
...
ExtMove *generateEvasionMoves(const Position &pos, ExtMove *mlist) {
  return pos.side_to_move() == BLACK
             ? generate_evasions<BLACK, All>(pos, mlist)
             : generate_evasions<WHITE, All>(pos, mlist);
}
...
ExtMove *generateChecksMoves(const Position &pos, ExtMove *mlist) {
  return pos.side_to_move() == BLACK
             ? generate_checks<GenType, BLACK, All>(pos, mlist)
             : generate_checks<GenType, WHITE, All>(pos, mlist);
}
...
ExtMove *generateMoves(const Position &pos, ExtMove *mlist, Square recapSq) {
  ...
  const bool All = (GenType == EVASIONS_ALL) || (GenType == CHECKS_ALL) ||
                   (GenType == LEGAL_ALL) || (GenType == NON_EVASIONS_ALL) ||
                   (GenType == RECAPTURES_ALL) ||
                   (GenType == QUIET_CHECKS_ALL) ||
                   (GenType == CAPTURES_PRO_PLUS_ALL) ||
  ...
  if (GenType == LEGAL || GenType == LEGAL_ALL) {
    ...
    auto last = pos.in_check() ? generateEvasionMoves<All>(pos, mlist)
                               : generateMoves<NON_EVASIONS, All>(pos, mlist);
  ...
  // 王手生成
  if (GenType == CHECKS || GenType == CHECKS_ALL || GenType == QUIET_CHECKS ||
      GenType == QUIET_CHECKS_ALL) {
  ...
  // 上記のもの以外
  // ただし、NON_EVASIONS_ALL ,
  // RECAPTURES_ALLは、ALLではないほうを呼び出す必要がある。
  // EVASIONS_ALLは上で呼び出されているが、実際はここでも実体化されたあと、最適化によって削除されるので、ここでも書く必要がある。
  const auto GenType2 = GenType == NON_EVASIONS_ALL        ? NON_EVASIONS
...
template ExtMove *generateMoves<NON_CAPTURES>(const Position &pos,
                                              ExtMove *mlist);
template ExtMove *generateMoves<CAPTURES>(const Position &pos, ExtMove *mlist);

template ExtMove *generateMoves<NON_CAPTURES_PRO_MINUS>(const Position &pos,
                                                        ExtMove *mlist);
template ExtMove *generateMoves<NON_CAPTURES_PRO_MINUS_ALL>(const Position &pos,
                                                            ExtMove *mlist);

template ExtMove *generateMoves<CAPTURES_PRO_PLUS>(const Position &pos,
                                                   ExtMove *mlist);
template ExtMove *generateMoves<CAPTURES_PRO_PLUS_ALL>(const Position &pos,
                                                       ExtMove *mlist);

template ExtMove *generateMoves<EVASIONS>(const Position &pos, ExtMove *mlist);
template ExtMove *generateMoves<EVASIONS_ALL>(const Position &pos,
                                              ExtMove *mlist);

template ExtMove *generateMoves<NON_EVASIONS>(const Position &pos,
                                              ExtMove *mlist);
template ExtMove *generateMoves<NON_EVASIONS_ALL>(const Position &pos,
                                                  ExtMove *mlist);
...
template ExtMove *generateMoves<CHECKS_ALL>(const Position &pos,
                                            ExtMove *mlist);

template ExtMove *generateMoves<QUIET_CHECKS>(const Position &pos,
                                              ExtMove *mlist);
template ExtMove *generateMoves<QUIET_CHECKS_ALL>(const Position &pos,
                                                  ExtMove *mlist);

template ExtMove *generateMoves<RECAPTURES>(const Position &pos, ExtMove *mlist,
                                            Square recapSq);
template ExtMove *generateMoves<RECAPTURES_ALL>(const Position &pos,
                                                ExtMove *mlist, Square recapSq);
